...
%  P_LED=5000;                     % Total transmitted power (mW)
...
EbN0 = [0:1:15];          % signal-to-noise ratio in dB.
% for i = 1:length(EbN0)
%     snr = EbN0(i);
% end
% 
...
%% Manggil file
%  namafile='PrxFix.xlsx';
%  sheetQ='BER_16QAM';
%  data=xlsread(namafile,sheetQ);
%  prx = data(1:30,4)/1000;
...
k = log2(M); % 2 pangkat
DC_bias = 16; % DC bias
bit = 1983996;% 61.440 for subcarrier = 64,128,256 ; Mapper = 16QAM,64QAM,QPSK ; NFFT = 128,256,512
iterasi =1;
...
%     data = 2*randi([0 1],NFFT,1)-1
% t_mod = comm.QPSKModulator;
...
%     t_clip = 0.5*(t_dc + abs(t_dc));
    ...
    %Snr = EbN0(ebno) 
%         p_vlc = t_dc*P_LED;
%         P_avg(ebno) = P_LED*H_A1.*sqrt(N0*Rb*SNR(ebno)/(2*R^2)); % average transmitted optical power
...
%         Tx_signal = t_dc.*i_peak(ebno);%
%            y1 = (Tx_signal);
%           y1 = filter(P_avg(ebno),1, Tx_signal);
%            y1 = filter(i_peak(ebno),1, Tx_signal);%
        %%kanal awgn
%          out = awgn(y1,snr,'measured');
...
mean_ber_ori = mean(bErr);
%  xlswrite('data_ber_QAM_ori', mean_ber_ori);
% 
%   semilogy(EbN0,bErr,'b-*','linewidth',1),grid on; hold on;
%   title('BER');
%   xlabel('Eb/No');
%   ylim([1e-4 1]);
